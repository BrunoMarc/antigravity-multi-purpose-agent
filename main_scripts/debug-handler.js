        ...
        try {
            switch (action) {
                ...
                case 'updateSchedule':
                    ...
                    if (params.fallbackModel !== undefined) await vscode.workspace.getConfiguration('auto-accept').update('fallbackModel', params.fallbackModel, vscode.ConfigurationTarget.Global);
                    return { success: true };

                case 'getSchedule':
                    const sched = vscode.workspace.getConfiguration('auto-accept.schedule');
                    const fallback = vscode.workspace.getConfiguration('auto-accept').get('fallbackModel', 'gemini 3.1');
                    return {
                        success: true,
                        schedule: {
